                ...
                if (r1.status == 200):
                    ...
                    if (len(meta_status) == 5):
                        ...
                        if fork_bomb_preventer:
                            print(
                                f"Error: Have tried to start more than {str(self.server_limit)} servers (the server limit) but according to the metaserver it has found none."
                            );
                        ...
                        for script in lt_scripts:
                            if script not in self.longturn:
                                ...
                                new_server = Civlauncher(
                                    ...
                                    script[10:-5],
                                    port,
                                    f"{metahost}:{str(metaport)}{metapath}",
                                    self.savesdir,
                                )
